...
<body>
<h1>USER_Recipe_detail</h1></br>
		<h3>레시피 명: ${recipe.rc_title}</h3>
		<h4>가격 대 : ${recipe.rc_type}</h4>
		<h4>등록 일 : <fmt:formatDate value="${recipe.rc_date}" pattern="yyyy-MM-dd"/></h4>
		<h4>조회 수 : ${recipe.rc_num}</h4>
		<h4>좋아요 :
			<c:if test="${!empty recipe.good_email}">
				<button style="background-color:CYAN" onclick="location.href='UserUnGoodClick.do?rc_idx=${recipe.rc_idx}&gubun=detail'">좋아요 눌림</button>
			</c:if>
			<c:if test="${empty recipe.good_email}">
				<c:if test="${ !empty loginUser}">
					<button onclick="location.href='UserGoodClick.do?rc_idx=${recipe.rc_idx}&gubun=detail'">조아효</button>
				</c:if>
				<c:if test="${ empty loginUser}">
			...
			<tr>
 			...
 			<c:forEach var="dto" items="${reply_list}">
  					...
  					<td>${dto.reply_date}</td> 
   					...
   					<c:choose>
						<c:when test="${loginUser.email eq dto.email}"><!-- eq EQual / == -->
							<td><a href="userdel.do?rc_idx=${dto.rc_idx}&reply_idx=${dto.reply_idx}&email=${recipe.email}&rc_title=${recipe.rc_title}">삭제</a></td>
						</c:when>
				...
				<tr class="show_review_tr" id="tr_head_${dto.reply_idx}">
					...
					<td><img src="../images/recipe/${recipe.email}/${recipe.rc_title}/${dto.reply_img}"></td>
				</tr>
...
<!-- 				맨 첫페이지 이동 -->
 		...
 		<form action="user_recipe_detail.do#pagination" method="post" id='frmPaging'>
...
<!-- 		<a href='write.do'>글쓰기</a> -->
	...
	<c:if test="${!empty loginUser}">
	<form name="input" method="post" action="write.do" enctype="multipart/form-data" id="reply_form">
	   <table border="0" width="600" align="center"  cellpadding="3" cellspacing="1" bordercolor="gray">
	  	<input type="hidden" name="write_gubun" value="MEMBER">
	   	...
	   	<input type="hidden" name="recipe_email" value="${recipe.email}">
	      <!-- <tr>
...
<!-- 		  </tr> -->
          <tr>
			 ...
			 <td><input type="text" class="reply_area" name="reply_sub" size="60"></td>
		  </tr>
		  <tr>
		     <td align="center">CONTENT</td>
			 <td><textarea  name="reply_cont" class="reply_area" rows="3" cols="60"></textarea></td>
		  </tr>
